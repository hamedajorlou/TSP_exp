...
'''
class DVAE_GCN(DVAE):
    def __init__(self, max_n, nvt, START_TYPE, END_TYPE, hs=501, nz=56, bidirectional=False, levels=2):
...
class DCN(DVAE):
    ...
    """        
    def __init__(self, max_n, nvt, START_TYPE, END_TYPE, in_dim: int, hid_dim: int, K: int, n_layers: int,
                bias: bool = True, hs= 501, nz=56, act = F.relu, last_act = None, bidirectional=False, vid=True):
        super(DCN, self).__init__(max_n, nvt, START_TYPE, END_TYPE, hs, nz, bidirectional, vid)
        self.in_d = in_dim
        self.hid_d = hid_dim
        self.out_d = hs
        self.nz = nz
        ...
        self.bias = bias
        self.bidr = bidirectional
        self.readout = nn.Linear(hs,nz)
    ...
    def encode(self, X, adj):
        ...
        myGSOs_t = GSO_maker(adj_t) 
        myGSOs = myGSOs.to('cuda')
        myGSOs_t = myGSOs_t.to('cuda')
        X = X.to('cuda')
        W = TensorDataset(X, myGSOs, myGSOs_t)
        W_dl = DataLoader(W, batch_size=1)
        ...
        for X_dl , GSO_dl, GSO_dl_t in W_dl:
            X_dl_t = X_dl
            for _ , conv in enumerate(self.convs[:-1]):
                X_dl = self.act(conv(X_dl, GSO_dl.squeeze()))
            X_out = self.convs[-1](X_dl, GSO_dl.squeeze())

            for _ , conv in enumerate(self.convs[:-1]):
                X_dl_t = self.act(conv(X_dl_t, GSO_dl_t.squeeze()))
            X_out_transp = self.convs[-1](X_dl_t, GSO_dl_t.squeeze())
            a , _ = torch.max(torch.cat((X_out[:, -1, :],X_out_transp[:, 0, :]), dim=0), dim = 0)

            if self.bidr:
                Last_node_reps , _ = torch.max(torch.cat((X_out[:, -1, :],X_out_transp[:, 0, :]), dim=0), dim=0)  
            else:
                Last_node_reps = X_out[:, -1, :]


            mu = self.readout(Last_node_reps) 
            var = self.readout(Last_node_reps) 

            # mu = torch.randn((1,56))
            # var = torch.randn((1,56))

            mean.append(mu.detach())
            logvar.append(var.detach())



        A = torch.stack(mean, axis=0)
        B = torch.stack(logvar, axis=0)
