...
sys.path.append('%s/software/enas' % os.path.dirname(os.path.dirname(os.path.realpath(__file__)))) 
sys.path.append('%s' % os.path.dirname(os.path.dirname(os.path.realpath(__file__)))) 
sys.path.append('%s/dvae' % os.path.dirname(os.path.dirname(os.path.realpath(__file__)))) 
sys.path.append('%s/src' % os.path.dirname(os.path.dirname(os.path.realpath(__file__)))) 
sys.path.append('%s/src1' % os.path.dirname(os.path.dirname(os.path.realpath(__file__)))) 
sys.path.append('%s/src2' % os.path.dirname(os.path.dirname(os.path.realpath(__file__)))) 
...
from models import *
from dvae.dagnn import DAGNN
from dvae.dagnn_bn import DAGNN_BN
...
# must specify
parser.add_argument('--data-name', default='final_structures6', help='graph dataset name')  #'final_structures6'
parser.add_argument('--save-appendix', default='DCN',
...
parser.add_argument('--data-dir', default='/scratch/hajorlou/bidr_results/',
                    help='where to save the Bayesian optimization results')
# BO settings
...
parser.add_argument('--in_dim',type=int ,default= 8, choices =[8,10])
...
parser.add_argument('--bo',type=int, default=1, choices=[0, 1],
...
data = loadmat(data_dir + '{}_latent_epoch_{}_{}.mat'.format(data_name, args.nz, checkpoint))  # load train/test data
...
for rand_idx in range(1, 11):
    ...
    model.to(device)
    load_module_state(model, data_dir + 'model_checkpoint_{}_{}.pth'.format(args.nz, checkpoint))
